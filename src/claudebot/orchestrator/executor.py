...
from claudebot.agents.task_graph import TaskGraph, TaskItem, TaskState
from claudebot.agents.base_agent import AgentResult
...
class Executor:
    ...
    async def _execute_parallel(self, tasks: List[TaskItem]) -> List[tuple]:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                processed_results.append((task, AgentResult(success=False, summary=str(result), error=str(result))))
            else:
    ...
    async def _execute_task(self, task: TaskItem) -> AgentResult:
        ...
        if not agent:
            ...
            return AgentResult(success=False, summary=task.error, error=task.error)

        # Build task dict for claudebot agents (async, dict-based interface)
        task_dict = {
            "id": task.id,
            "name": task.name,
            "description": task.input_data if isinstance(task.input_data, str) else (task.description or ""),
        }

        for attempt in range(self.retry_attempts):
            try:
                result = await agent.execute(task_dict, task.metadata.get("context"))
...
def create_executor(max_workers: int = 4, retry_attempts: int = 3) -> Executor:
